...
def main():
    ...
    parser.add_argument("--draft-type", type=str, required=True, help="Tipo: reply_1, reply_2, quote, new_post")
    parser.add_argument("--action", type=str, required=True, help="Acción: approved, rejected, edited, published")
    parser.add_argument("--notes", type=str, default="", help="Notas opcionales")

    args = parser.parse_args()

    if args.draft_type not in VALID_DRAFT_TYPES:
        raise ValueError(f"draft_type inválido. Usa uno de: {', '.join(sorted(VALID_DRAFT_TYPES))}")

    if args.action not in VALID_ACTIONS:
        raise ValueError(f"action inválida. Usa una de: {', '.join(sorted(VALID_ACTIONS))}")
